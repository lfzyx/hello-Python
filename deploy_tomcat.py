...
for sections in configfile.__get_sections__():
    '''
    根据不通的配置项打开工程中不通的配置文件，然后修改能匹配的配置项属性值
    '''
    temp = ""
    try:
        with open(os.path.join(docBase, project, configfile.__get_items__(sections, "path")), "r", encoding="utf-8") \
                as file:
            for line in file:
                _list = line.split('=')
                for item in configfile.__get_options__(sections):
                    if (_list[0] == item):
                        _list[1] = configfile.__get_items__(sections, item)
                        line = _list[0] + '=' + _list[1] + '\n'
                temp += line
    except (IsADirectoryError, FileNotFoundError):
        print(sys.exc_info()[1])
    else:
        with open(os.path.join(docBase, project, configfile.__get_items__(sections, "path")), "w", encoding="utf-8") \
                as file:
            file.write(temp)
